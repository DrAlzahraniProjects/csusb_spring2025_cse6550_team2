...
def generate_response_with_timeout(user_input: str, cache_key: str, messages, vectorstore) -> str:
    with Manager() as manager:
        ...
        with Pool(processes=1) as pool:
            result = pool.apply_async(generate_response_worker, (user_input, cache_key, messages, vectorstore, return_dict))
            try:
                result.get(timeout=MAX_RESPONSE_TIME)  # 注意这里不返回response了
