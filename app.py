...
class ResponseGenerator:
    def __init__(self):
        self.timeout_seconds = MAX_RESPONSE_TIME

    def generate_response_with_timeout(self, user_input: str, cache_key: str, messages) -> str:
        result_queue = Queue()
        p = Process(target=self._worker_process, args=(user_input, cache_key, messages, result_queue))
        p.start()
        ...
        return result_queue.get() if not result_queue.empty() else TIMEOUT_MESSAGE

    @staticmethod
    def _worker_process(user_input: str, cache_key: str, messages, result_queue: Queue):
        try:
            from langchain_community.embeddings import FastEmbedEmbeddings
            from langchain_groq import ChatGroq
            from langchain_community.vectorstores import FAISS
            ...
            vectorstore = getInitialVectorstore()
            if cache_key in ANSWER_CACHE:
                result_queue.put(ANSWER_CACHE[cache_key][1])
                return

            semantic_match = find_semantic_match(user_input)
            if semantic_match:
                result_queue.put(semantic_match[0])
                return
            ...
            ai = ChatGroq(model="llama-3.1-8b-instant", temperature=0.1, api_key=os.environ.get("GROQ_API_KEY"))
            response = ai.invoke(truncated_messages).content
            embedding = embedding_model.embed_query(user_input)
            ...
            result_queue.put(f"System error: {str(e)}")
...
def mainPage():
    ...
    if user_input and canAnswer():
        ...
        with st.chat_message("ai"):
            ...
            response = TIMEOUT_MESSAGE  # default

            try:
                messages_copy = st.session_state["messages"].copy()
                vectorstore = st.session_state.get("vectorstore", None)
                response_generator = ResponseGenerator()
                response = response_generator.generate_response_with_timeout(
                    user_input, cache_key, messages_copy
                )
